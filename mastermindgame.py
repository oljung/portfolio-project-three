...
class RunMastermind:
    ...
    def prepare_highscore_item(self):
        ...
        if len(self.result_list) == 0 or self.custom:
            return False
        else:
            # average score of rounds played, rounded to two decimals
            average_score = round(sum(self.result_list)/len(self.result_list), 2)
            return [self.name, len(self.result_list), self.best_score,
                    self.worst_score, average_score]
